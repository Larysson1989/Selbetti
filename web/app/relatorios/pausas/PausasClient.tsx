...
export default function PausasClient() {
  ...
  const [resultado, setResultado] = useState<PausaRow[] | null>(null);
  ...
  async function buscar() {
    ...
    try {
      const res = await fetch(
        `/api/relatorios/pausas?idExterno=${idExterno}&dataIni=${dataIni}&dataFim=${dataFim}`
      );
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="font-display text-xl mb-1">Pausas por período</h2>
        <p className="text-sm text-muted">
          Histórico de pausas dos agentes, por campanha.
        </p>
      </div>
        ...
        <div className="bg-panel border border-panel-border rounded-lg overflow-x-auto">
          {resultado.length === 0 ? (
            <div className="px-6 py-10 text-center text-sm text-muted">
              Nenhuma pausa encontrada para {nomeCampanha} neste período.
            </div>
          ) : (
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wider text-muted border-b border-panel-border">
                  ...
                  <th className="px-4 py-3 font-normal">Pausa</th>
              ...
              <tbody className="font-mono text-xs">
                {resultado.map((r, i) => (
                  <tr key={i} className="border-b border-panel-border last:border-0">
                    ...
                    <td className="px-4 py-2 font-sans">{r.descricao}</td>
                    <td className="px-4 py-2">{r.inicio}</td>
                    <td className="px-4 py-2">{r.fim}</td>
                  </tr>
                ))}
              </tbody>
            </table>
